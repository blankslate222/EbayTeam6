...
(function($) {
	...
	$.fn.AnyTime_setEarliest = function(options) {
		return this.each(function(i) {
			AnyTime.setEarliest(this.id, options);
		});
	}

	$.fn.AnyTime_setLatest = function(options) {
		return this.each(function(i) {
			AnyTime.setLatest(this.id, options);
		});
	}
	...
	AnyTime.Converter = function(options) {
		...
		this.dNames = [ 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday',
				'Friday', 'Saturday' ];
		this.eAbbr = [ 'BCE', 'CE' ];
		this.mAbbr = [ 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug',
				'Sep', 'Oct', 'Nov', 'Dec' ];
		this.mNames = [ 'January', 'February', 'March', 'April', 'May', 'June',
		...
		this.dAt = function(str, pos) {
			return ((str.charCodeAt(pos) >= '0'.charCodeAt(0)) && (str
					.charCodeAt(pos) <= '9'.charCodeAt(0)));
		};
				...
				if (this.fmt.charAt(f) != '%')
					...
					case '#': // signed timezone offset in minutes
						t = (_offAl != Number.MIN_VALUE) ? _offAl
								: (_offF == Number.MIN_VALUE) ? (0 - d
										.getTimezoneOffset()) : _offF;
						if (t >= 0)
							str += '+';
						str += t;
						break;
					case '@': // timezone offset label
						...
						if (AnyTime.utcLabel && AnyTime.utcLabel[t]) {
							if ((_offFSI > 0)
									&& (_offFSI < AnyTime.utcLabel[t].length))
					...
					case ';': // signed 3-or-4-digit timezone offset with colon delimiter
						...
						if (t < 0)
							...
							str += '+';
						t = Math.abs(t);
						str += ((ch == '+') || (ch == ':')) ? AnyTime.pad(Math
								.floor(t / 60), 2) : Math.floor(t / 60);
						if ((ch == ':') || (ch == ';'))
							str += ':';
		...
		this.parse = function(str) {
			...
			if (tzOff != Number.MIN_VALUE) {
				if (_captureOffset)
					...
					time.setTime((time.getTime() - (tzOff * 60000))
	...
	AnyTime.picker = function(id, options) {
		...
		__pickers[id] = {
			...
			initialize : function(id) {
				...
				this.id = 'AnyTime--'
						+ id.replace(/[^-_.A-Za-z0-9]/g, '--AnyTime--');
				...
				if (options.earliest) {
					if (typeof options.earliest.getTime == 'function')
						...
						this.earliest = this.conv.parse(options.earliest
								.toString());
				}

				if (options.firstDOW) {
					if ((options.firstDOW < 0) || (options.firstDOW > 6))
						throw new Exception('illegal firstDOW: '
								+ options.firstDOW);
				...
				if (options.latest) {
					if (typeof options.latest.getTime == 'function')
						...
						this.latest = this.conv
								.parse(options.latest.toString());
				}
				...
				var askSec = ((format.indexOf('%r') >= 0)
						|| (format.indexOf('%S') >= 0)
						|| (format.indexOf('%s') >= 0) || (format.indexOf('%T') >= 0));
				...
				var xDiv = null;
				if (this.pop) {
					xDiv = $('<div class="AnyTime-x-btn ui-state-default">'
							+ this.lX + '</div>');
				...
				if (askDate) {
					...
					if (askYear) {
						this.yLab = $('<h6 class="AnyTime-lbl AnyTime-lbl-yr">'
								+ this.lY + '</h6>');
					...
					if (askMonth) {
						...
						this.hMo = $('<h6 class="AnyTime-lbl AnyTime-lbl-month">'
								+ lab + '</h6>');
					...
					if (askDoM) {
						...
						this.hDoM = $('<h6 class="AnyTime-lbl AnyTime-lbl-dom">'
								+ lab + '</h6>');
						...
						for (var r = 0; r < 6; r++) {
							tr = $('<tr class="AnyTime-wk AnyTime-wk'
									+ String(r + 1) + '"/>');
							tbody.append(tr);
							for (i = 0; i < 7; i++)
				...
				if (askTime) {
					...
					if (askHour) {
						...
						this.dH
								.append($('<h6 class="AnyTime-lbl AnyTime-lbl-hr">'
										+ lab + '</h6>'));
						...
						for (i = 0; i < 12; i++) {
							...
							this.btn(amDiv, t, this.newHour, [ 'hr',
									'hr' + String(i) ], lab + ' ' + t);
							...
							this.btn(pmDiv, t, this.newHour, [ 'hr',
									'hr' + String(i + 12) ], lab + ' ' + t);
						}
					...
					if (askMinute) {
						...
						this.dM
								.append($('<h6 class="AnyTime-lbl AnyTime-lbl-min">'
										+ lab + '</h6>'));
						...
						for (i = 0; i < 10; i++)
							this.btn(onesDiv, i, function(event) {
								...
								var t = new Date(this.time.getTime());
								t
										.setMinutes((Math.floor(this.time
												.getMinutes() / 10) * 10)
												+ Number(elem.text()));
								this.set(t);
								this.upd(elem);
					...
					if (askSec) {
						...
						this.dS
								.append($('<h6 class="AnyTime-lbl AnyTime-lbl-sec">'
										+ lab + '</h6>'));
						...
						for (i = 0; i < 10; i++)
							this.btn(onesDiv, i, function(event) {
								...
								var t = new Date(this.time.getTime());
								t
										.setSeconds((Math.floor(this.time
												.getSeconds() / 10) * 10)
												+ Number(elem.text()));
								this.set(t);
								this.upd(elem);
					...
					if (askOff) {
						...
						this.oCur = this.btn(this.oList, '', this.newOffset, [
								'off', 'off-cur' ], lab);
						...
						this.oSel = this.btn(this.oList, '&#177;',
								this.newOffset, [ 'off', 'off-select' ], '+/- '
										+ this.lO);

						this.oMinW = this.dO.outerWidth(true);
						this.oLab = $('<h6 class="AnyTime-lbl AnyTime-lbl-off">'
								+ this.lO + '</h6>');
				...
				if (options.labelTitle)
					...
					this.hTitle.append('Select a '
							+ (askDate ? (askTime ? 'Date and Time' : 'Date')
									: 'Time'));
				else
				...
				if (this.ajaxOpts
					...
					try {
						...
						if (typeof opts.data == 'object')
							opts.data[this.inp[0].name || this.inp[0].id] = this.inp
									.val();
						else {
							var opt = (this.inp[0].name || this.inp[0].id)
			...
			askOffset : function(event) {
				if (!this.oDiv) {
					...
					var xDiv = $('<div class="AnyTime-x-btn ui-state-default">'
							+ this.lX + '</div>');
					title.append(xDiv);
					xDiv.click(function(e) {
						_this.dismissODiv(e);
					...
					if (this.oDiv.outerHeight(true) > this.div.height()) {
						...
						this.oBody.css({
							height : String(this.div.height()
									- (this.oDiv.outerHeight(true) + this.oBody
											.outerHeight(false)))
									+ 'px',
					...
					if (this.oDiv.outerWidth(true) > this.div.width()) {
						...
						this.oBody.css('width', String(this.div.width()
								- (this.oDiv.outerWidth(true) + this.oBody
										.outerWidth(false)))
								+ 'px');
					}
			...
			askYear : function(event) {
				if (!this.yDiv) {
					...
					if (this.askEra) {
						...
						this.btn(cont, this.conv.eAbbr[0], function(event) {
							var t = new Date(this.time.getTime());
						...
						this.btn(cont, this.conv.eAbbr[1], function(event) {
							var t = new Date(this.time.getTime());
							var year = t.getFullYear();
							if (year < 0)
								t.setFullYear(0 - year);
			...
			inpBlur : function(event) {
				...
				setTimeout(function() {
					if (_this.lostFocus) {
						_this.div.find('.AnyTime-focus-btn').removeClass(
			...
			btn : function(parent, text, handler, classes, title) {
				var tagName = ((parent[0].nodeName.toLowerCase() == 'ul') ? 'li'
						: 'td');
				var div$ = '<' + tagName + ' class="AnyTime-btn';
				for (var i = 0; i < classes.length; i++)
					div$ += ' AnyTime-' + classes[i] + '-btn';
				var div = $(div$ + ' ui-state-default">' + text + '</'
						+ tagName + '>');
				parent.append(div);
			...
			cleanup : function(event) {
				this.inp.prop('readonly', this.ro).off('blur', this.hBlur).off(
						'click', this.hClick).off('focus', this.hFocus).off(
						'keydown', this.hKeydown).off('keypress',
						this.hKeypress);
			...
			setFocus : function(btn) {
				...
				if (btn.hasClass('AnyTime-off-off-btn')) {
					...
					if (btnT - btnH < oBT) // move a page up
						this.oBody
								.scrollTop(btnT + this.oBody.scrollTop()
			...
			key : function(event) {
				...
				} else if ((key == 34) || (key == 9)) // PageDown or Tab
				{
					if (this.fBtn.hasClass('AnyTime-mil-btn'))
						...
						t = this.dDoM || this.dMo || this.dH || this.dM
								|| this.dS || this.dO;
						if (t)
							t.AnyTime_clickCurrent();
						else if (key == 9) {
			...
			newYPos : function(event) {
				...
				if (elem.hasClass('AnyTime-mil-btn'))
					...
					year = year.substring(0, 1) + elem.html()
							+ year.substring(2, 4);
				else if (elem.hasClass('AnyTime-dec-btn'))
					year = year.substring(0, 2) + elem.html()
							+ year.substring(3, 4);
				else
			...
			pos : function(event) // note: event is ignored but this is a handler
			{
				...
				if (this.oDiv && this.oDiv.is(":visible")) {
					...
					yOff.left += ((this.yLab.outerWidth(true) - this.yDiv
							.outerWidth(true)) / 2);
					this.yDiv.css({
						top : yOff.top + 'px',
						left : yOff.left + 'px'
			...
			upd : function(fBtn) {
				...
				if (this.yPrior) {
					this.yPrior.text(AnyTime.pad((current == 1) ? (-1)
							: (current - 1), 4));
					if (this.earliest) {
				...
				if (this.yNext) {
					this.yNext.text(AnyTime.pad((current == -1) ? 1
							: (current + 1), 4));
					if (this.latest) {
						...
						.each(
								function() {
									...
									$(this)
											.AnyTime_current(
													i == current,
													((!_this.earliest) || (cmpHi
															.getTime() >= _this.earliest))
															&& ((!_this.latest) || (cmpLo
																	.getTime() <= _this.latest)));
									i++;
								});
						...
						.each(
								function() {
									...
									if (cmpLo.getMinutes() < 50) {
										cmpLo
												.setMinutes(cmpLo.getMinutes() + 10);
										cmpHi
												.setMinutes(cmpHi.getMinutes() + 10);
									}
								});
						...
						.each(
								function() {
									...
									if (cmpLo.getSeconds() < 50) {
										cmpLo
												.setSeconds(cmpLo.getSeconds() + 10);
										cmpHi
												.setSeconds(cmpHi.getSeconds() + 10);
									}
								});
			...
			updYDiv : function(fBtn) {
				...
				var eY = _this.earliest
						&& new Date(_this.earliest).getFullYear();
				...
				this.yDiv
						...
						.each(
								function() {
									legal = (((!_this.earliest) || (era
											* (i + (era < 0 ? 0 : 999)) >= eY)) && ((!_this.latest) || (era
											* (i + (era > 0 ? 0 : 999)) <= lY)));
									$(this).AnyTime_current(
											this.innerHTML == yearValue
													.substring(0, 1), legal);
									i += 1000;
								});
				...
				this.yDiv
						...
						.each(
								function() {
									legal = (((!_this.earliest) || (era
											* (i + (era < 0 ? 0 : 99)) >= eY)) && ((!_this.latest) || (era
											* (i + (era > 0 ? 0 : 99)) <= lY)));
									$(this).AnyTime_current(
											this.innerHTML == yearValue
													.substring(1, 2), legal);
				...
				this.yDiv
						...
						.each(
								function() {
									legal = (((!_this.earliest) || (era
											* (i + (era < 0 ? 0 : 9)) >= eY)) && ((!_this.latest) || (era
											* (i + (era > 0 ? 0 : 9)) <= lY)));
									$(this).AnyTime_current(
											this.innerHTML == yearValue
													.substring(2, 3), legal);
				...
				this.yDiv
						...
						.each(
								function() {
									legal = (((!_this.earliest) || (era * i >= eY)) && ((!_this.latest) || (era
											* i <= lY)));
									$(this).AnyTime_current(
											this.innerHTML == yearValue
													.substring(3), legal);
